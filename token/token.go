...
// Note: the footer of the token is never encrypted.
func (t *token) Encrypt(secret, implict []byte) (string, error) {
...
func (t *token) Refresh() error {
	...
	if err != nil {
		return fmt.Errorf(
			"failed to get exp from token: %s",
			err.Error(),
		)
	}
	...
	if err != nil {
		return fmt.Errorf(
			"failed to get iat from token: %s",
			err.Error(),
		)
	}
